...
describe("admin operations helpers", () => {
  ...
  it("filters shipments and edit requests consistently", () => {
    expect(
      filterShipments([{ trackingId: "TRK-1", dealNumber: "DL-1", clientName: "Acme", destination: "Riyadh", stage: "delivered" }] as any, "riyadh"),
    ).toHaveLength(1);

    expect(
      filterFinancialEditRequests(
        [{ requestedBy: "Mona", requestedByEmail: "mona@example.com", dealNumber: "DL-1", targetEntryNumber: "FE-1", reason: "Amount correction", status: "pending" }] as any,
        "amount",
        "pending",
      ),
    ).toHaveLength(1);
  ...
  it("builds csv exports for reports and accounting entries", () => {
    const reportCsv = buildReportCsv({
      ...
      topCustomers: [{ fullName: "Acme", outstandingBalance: 200, pendingEditRequests: 1 }] as any,
