    ...
    return (
      <div className="rounded-[1.35rem] border border-amber-200/10 bg-stone-900/50 p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <p className="font-medium text-stone-100">{isAr ? "قائمة التعبئة" : "Packing list"}</p>
        </div>
        <p className="text-sm text-stone-500">
          {isAr ? "لم يتم حفظ قائمة تعبئة لهذه الصفقة بعد." : "No packing list saved for this deal yet."}
        </p>
      </div>
    );
  }
      ...
      <div className="border-t border-amber-200/10 pt-3">
        <div className="flex items-center justify-between text-xs">
          <span className="text-stone-600">
            {isAr ? "تم التقديم بواسطة" : "Submitted by"}:{" "}
            <span className="text-stone-400">{packingList.submittedByRole}</span>
          </span>
          <span className="text-stone-600">
            {packingList.createdAt
              ? new Date(packingList.createdAt).toLocaleDateString(lang === "ar" ? "ar-SA" : "en-US")
              : "—"}
          </span>
        </div>
      </div>
