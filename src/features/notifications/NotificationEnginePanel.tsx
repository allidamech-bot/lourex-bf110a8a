...
import { AlertTriangle, BellRing, CheckCircle2, Mail, MessageCircle, RefreshCw, Send, Settings2 } from "lucide-react";
import BentoCard from "@/components/BentoCard";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { loadNotificationEngineReport, type NotificationDeliveryQueueRow, type NotificationEngineReport, type NotificationTemplateRow } from "@/domain/notifications/notificationEngine";

const dateTime = (value: string | null | undefined) => (value ? new Date(value).toLocaleString() : "Not recorded");

const statusClass = (value: string) => {
  if (["sent", "ready", "enabled"].includes(value)) return "border-emerald-500/30 bg-emerald-500/10 text-emerald-300";
  if (["queued", "processing", "provider_not_configured", "warning"].includes(value)) return "border-amber-500/30 bg-amber-500/10 text-amber-300";
