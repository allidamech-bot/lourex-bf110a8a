import { useEffect, useMemo, useState } from "react";
import { Link, useSearchParams } from "react-router-dom";
import { AlertCircle, ArrowLeft, RefreshCw } from "lucide-react";

import CustomerTrackingProView from "@/features/customer/CustomerTrackingProView";
import { EmptyState } from "@/components/shared/EmptyState";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { useAuthSession } from "@/features/auth/AuthSessionProvider";
import { useI18n } from "@/lib/i18n";
import { loadShipments } from "@/lib/operationsDomain";
import { logOperationalError } from "@/lib/monitoring";

type CustomerShipmentRows = Awaited<ReturnType<typeof loadShipments>>;
type CustomerShipmentRow = CustomerShipmentRows[number];
type ShipmentOwnershipFields = {
  customerEmail?: string | null;
  customer_email?: string | null;
  customer?: {
    email?: string | null;
  } | null;
};

const CustomerTrackingProPage = () => {
  const { profile } = useAuthSession();
  const { locale } = useI18n();
  const [searchParams, setSearchParams] = useSearchParams();
  const [shipments, setShipments] = useState<CustomerShipmentRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState("");

  const lang = locale === "ar" ? "ar" : "en";
  const selectedTracking = searchParams.get("tracking");
  const normalizedEmail = profile?.email?.trim().toLowerCase() || "";

  const loadData = async (mode: "initial" | "refresh" = "initial") => {
    if (mode === "initial") setLoading(true);
    else setRefreshing(true);

    setError("");

    try {
      const data = await loadShipments();
      const visibleShipments = normalizedEmail
        ? data.filter((row: CustomerShipmentRow & ShipmentOwnershipFields) => {
            const rowEmail = (row.customerEmail || row.customer_email || row.customer?.email || "").trim().toLowerCase();
            return !rowEmail || rowEmail === normalizedEmail;
          })
        : data;

      setShipments(visibleShipments);

      if (!selectedTracking && visibleShipments[0]) {
        setSearchParams({ tracking: visibleShipments[0].trackingId }, { replace: true });
      }
    } catch (err) {
      logOperationalError("customer_tracking_pro_load", err, { customerId: profile?.id });
      setError(lang === "ar" ? "تعذر تحميل عرض التتبع الاحترافي." : "Unable to load the tracking pro view.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  };

  useEffect(() => {
    void loadData("initial");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profile?.id, normalizedEmail, lang]);

  const selectedShipment = useMemo(() => {
    if (!shipments.length) return null;
    return shipments.find((shipment) => shipment.trackingId === selectedTracking) || shipments[0];
  }, [shipments, selectedTracking]);

  if (loading) {
    return (
      <div className="space-y-4">
        <Skeleton className="h-32 rounded-[1.75rem] bg-stone-900" />
        <Skeleton className="h-[640px] rounded-[1.75rem] bg-stone-900" />
      </div>
    );
  }

  return (
    <div className="space-y-5 pb-12">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <Button asChild variant="link" className="h-auto p-0 text-amber-300">
            <Link to="/customer-portal/tracking">
              <ArrowLeft className="me-2 h-4 w-4" />
              {lang === "ar" ? "العودة إلى التتبع" : "Back to tracking"}
            </Link>
          </Button>
          <p className="mt-4 text-xs font-semibold uppercase tracking-[0.24em] text-amber-500/80">
            {lang === "ar" ? "تتبع الشحنة" : "Shipment tracking"}
          </p>
          <h1 className="mt-2 font-serif text-3xl font-bold text-stone-100 md:text-4xl">
            {lang === "ar" ? "العرض الاحترافي للتتبع" : "Customer Tracking Pro View"}
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-stone-400">
            {lang === "ar"
              ? "صفحة واحدة تعرض المرحلة الحالية، مسار الشحنة، آخر تحديث رسمي، الملاحظات، وروابط الإجراءات."
              : "One page for the current stage, shipment journey, latest official update, notes, and action links."}
          </p>
        </div>
        <Button
          variant="outline"
          className="border-amber-200/15 bg-stone-50/5 text-stone-100 hover:bg-stone-50/10"
          onClick={() => void loadData("refresh")}
          disabled={refreshing}
        >
          <RefreshCw className={`me-2 h-4 w-4 ${refreshing ? "animate-spin text-amber-500" : "text-amber-500"}`} />
          {refreshing ? (lang === "ar" ? "جاري التحديث..." : "Refreshing...") : (lang === "ar" ? "تحديث" : "Refresh")}
        </Button>
      </div>

      {error ? (
        <div className="flex items-start gap-3 rounded-[1.5rem] border border-destructive/20 bg-destructive/10 px-4 py-4 text-sm text-destructive">
          <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
          <span>{error}</span>
        </div>
      ) : null}

      {selectedShipment ? (
        <CustomerTrackingProView shipment={selectedShipment} locale={locale} />
      ) : (
        <EmptyState
          icon={AlertCircle}
          title={lang === "ar" ? "لا توجد شحنة محددة" : "No shipment selected"}
          description={lang === "ar" ? "ارجع إلى صفحة التتبع أو انتظر تحويل طلبك إلى عملية." : "Return to tracking or wait until your request is converted into an operation."}
        />
      )}
    </div>
  );
};

export default CustomerTrackingProPage;
