-- Phase 10 stabilization: customer RPC reliability and transfer proof ownership/RLS repair.
...
begin
  for v_constraint_name in
    select conname
    from pg_constraint
    where conrelid = 'public.transfer_proofs'::regclass
      and contype = 'f'
      and conkey = array[
        (
          select attnum
          from pg_attribute
          where attrelid = 'public.transfer_proofs'::regclass
            and attname = 'customer_id'
        )
      ]::smallint[]
  loop
    execute format('alter table public.transfer_proofs drop constraint if exists %I', v_constraint_name);
  end loop;
  ...
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'public.transfer_proofs'::regclass
      and conname = 'transfer_proofs_customer_id_fkey'
  ) then
    alter table public.transfer_proofs
      add constraint transfer_proofs_customer_id_fkey
      foreign key (customer_id) references public.lourex_customers(id) on delete cascade;
  end if;
end $$;
...
with check (
  ...
  and exists (
    ...
    where pr.id = transfer_proofs.request_id
      and pr.customer_id = transfer_proofs.customer_id
...
using (
  ...
  or exists (
    select 1
    from public.lourex_customers c
    where c.id = transfer_proofs.customer_id
      and lower(c.email) = lower(coalesce(auth.jwt() ->> 'email', ''))
  )
);
