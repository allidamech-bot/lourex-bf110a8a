-- Final transfer-proof RPC diagnostics and ownership repair.
--
-- Temporary structured diagnostics are returned through PostgREST error details
-- so authenticated browser failures show the exact gate: AUTH_REQUIRED,
-- REQUEST_NOT_FOUND, CUSTOMER_NOT_RESOLVED, NOT_OWNER, INVALID_STATUS, or INSERT_FAILED.
...
with check (
  ...
  and exists (
    ...
    where pr.id = transfer_proofs.request_id
      and pr.status in ('ready_for_conversion', 'transfer_uploaded', 'transfer_proof_rejected', 'transfer_proof_pending')
      and (
...
with check (
  ...
  and exists (
    ...
    where pr.id = split_part(storage.objects.name, '/', 3)::uuid
      and pr.status in ('ready_for_conversion', 'transfer_uploaded', 'transfer_proof_rejected')
      and (
